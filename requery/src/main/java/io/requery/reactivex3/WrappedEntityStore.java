...
class WrappedEntityStore<T> extends ReactiveEntityStore<T> {
    ...
    @Override
    public <E extends T> InsertInto<ReactiveResult<Tuple>> insert(Class<E> type, QueryAttribute<?, ?>... attributes) {
    ...
    public <R> Single<R> runInTransaction(final Function<BlockingEntityStore<T>, R> function) {
        return Single.fromCallable(new Callable<R>() {
            ...
            public R call() throws Exception {
                return delegate.runInTransaction(new Callable<R>() {
                    @Override
                    public R call() throws Exception {
    ...
    private static <E> QueryElement<ReactiveResult<E>> result(Return<? extends Result<E>> query) {
        final QueryElement<Result<E>> element = (QueryElement<Result<E>>) query;
        return element.extend(new io.requery.util.function.Function<Result<E>, ReactiveResult<E>>() {
            @Override
    ...
    private static <E> QueryElement<ReactiveScalar<E>> scalar(Return<? extends Scalar<E>> query) {
        final QueryElement<Scalar<E>> element = (QueryElement<Scalar<E>>) query;
        return element.extend(new io.requery.util.function.Function<Scalar<E>, ReactiveScalar<E>>() {
            @Override
